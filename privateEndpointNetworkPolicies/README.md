...
## Appendix: verify PE traffic is hitting Azure Firewall

After flipping a PE subnet to `RouteTableEnabled` and pointing a UDR at Azure Firewall, you can confirm the traffic is actually flowing through AzFW with a Log Analytics / Sentinel KQL query against the firewall's diagnostic logs (`AzureDiagnostics` table; works for both classic and structured logs as long as `AZFWNetworkRule` category is enabled).

Replace the IP with the PE's private IP from `private-endpoints.tsv` (the `PrivateIP` column).

```kql
AzureDiagnostics
| where Category contains "AZFWNetworkRule"
| where DestinationIp_s has "10.0.92.4"
| where Action_s has "Allow"
```

Tips:

- **No rows** = either the UDR isn't pointing at AzFW, the firewall hasn't logged anything yet, or the subnet's `privateEndpointNetworkPolicies` is still `Disabled` (UDR ignored).
- For application-layer hits, swap `AZFWNetworkRule` for `AZFWApplicationRule` and filter on `Fqdn_s` (e.g. `<storageaccount>.blob.core.windows.net`).
- To check denied traffic, change `Action_s has "Allow"` to `"Deny"`.
- Add a time window for noisy environments, e.g. `| where TimeGenerated > ago(1h)`.
